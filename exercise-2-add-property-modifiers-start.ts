...
interface Country {
  name: string;
  code: string;
  population?: number; // Aggiungo il modificatore di proprietà "?" per rendere la proprietà `population` opzionale.
}
...
type Currency = {
  readonly name: string; // Aggiungo il modificatore di proprietà "readonly" per rendere la proprietà `name` non modificabile.
  code: string;                 // Provare a modificare una proprietà "readonly" genererà un errore di tipo.
  symbol: string;
};
